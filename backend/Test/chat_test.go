...
func TestGroupChat(t *testing.T) {
	...
	groupId, err := group.CreateGroup("hello", "testing", result1.UserId, database)
	fmt.Println(groupId)
	...
	_, err = chat.GetUserGroups(result1.UserId, database)
	t.Run("Get user groups", func(t *testing.T) {
		if err != nil {
			t.Errorf("Error getting user groups %v", err)
		}
	})
	_, err = chat.GetAllMembersOfGroup(groupId, database)
